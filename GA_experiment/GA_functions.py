...
def conc_to_spectra(conc, stock_spectra):
    ''' Simulates Beer's law and with concentrations of differnt stocks and the spectra of these stocks. 
    
        Inputs:
       - conc: A 2-D array with the number of columns equal to the number of stocks and rows equal to the
               number of different combinations of stocks to be tested. 
               
       - stock_spectra: A 2-D array with the number of columns equal to the number of stocks and the rows
               equal to the number of absorabance datapoints.
               
        Outputs:
        
       - spectra_final: A 2-D array with the number of columns equal to the number of absorbance datapoints
               and the number of rows equal to the number of combinations of stocks that were tested.     
    '''
    
    for i in range(conc.shape[0]):
        for j in range(stock_spectra.shape[1]):
            #np.random.seed(i*j+1)
            spectra_part = conc[i,j]*stock_spectra[:,j] #*(1+np.random.rand(1)-0.5)/5
            if j == 0:
...
def fitness(spectra, conc, desired):
    ...
    #desired = MinMaxScaler().fit(desired).transform(desired).T
    ...
    for i in range(n_offspring):
        ...
        for n_stocks in range(parents.shape[1]-1):
            ...
            def cross_parents(p1_conc, p2_conc):    
                ...
                offspring_conc = float(zero + decimal + digit1 + digit2 + digit3)
...
def GA_algorithm(x_train_spectra, y_train_conc, x_test, n_parents, n_offspring, mutation_rate, fitness_param):
    np.random.seed(seed)
    #'Obtains fitness of the input concentrations and their spectra'
    array, median_fitness, max_fitness = fitness(x_train_spectra, y_train_conc, x_test)
    
    #'Obtains the parents of the ordered fitness array'
    parents = select_parents(array, n_parents, fitness_param)
...
def GA_algorithm_unnormalized(x_train_spectra, y_train_conc, x_test, n_parents, n_offspring, mutation_rate, fitness_param):
    np.random.seed(seed)
    #'Obtains fitness of the input concentrations and their spectra'
    array, median_fitness, max_fitness = fitness(x_train_spectra, y_train_conc, x_test)
...
def perform_iteration(current_gen_spectra, current_gen_conc, desired_spectra, n_parents, n_offspring, mutation_rate, fitness_multiplier):
    ''' Perfroms one iteration of the GA algorithm. 
    
    Inputs:
    - current_gen_spectra: The spectra of the current generation (batch). It is a 2D array with the number of rows
      equal to the number of samples in the generation and number of colums equal to the number of spectra datapoints.
    - current_gen_conc: The concentration of the current generation (batch). It is a 2D array with the number of rows
      equal to the number of samples in the generation and the number of columns equal to the number of dimensions,
      for exmaple, 3 columns if we are mixing red, blue, green dyes.
    - desired_spectra: The desired spectra. It is a 1D array with one row and number of columns equal to the number of     datapoints
      in the spectra.
    - n_parents: Integer which determines how many parents to create from the current generation.
    - n_offspring: Integer which determines how many offspring to create from the current generation.
    - mutation_rate: Float from range 0-1 which determines how often a mutation occurs. 
    - fitness_multiplier: Float from range of 0-5 (rule of thumb), that increases the chance of more fit parents to be 
      selected.
      
    Outputs:
    - next_gen_conc: The concentrations of the next generation to be tested. It is a 2D array with number of rows 
      equal to n_offspring and number of columns equal to the number of dimensions. '''
    np.random.seed(seed)
    ...
    next_gen_conc, median_fitness, max_fitness = GA_algorithm(current_gen_spectra, current_gen_conc, desired_spectra, n_parents, n_offspring, mutation_rate, fitness_multiplier)
