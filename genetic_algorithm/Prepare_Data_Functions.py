...
def plot_some_spectra_single(df, cols):    
    ...
    for i in range(0, len(cols)):
        labels = ['0 PEG chains/nm$^2$', '0.16 PEG chains/nm$^2$', ' 0.32 PEG chains/nm$^2$', '0.65 PEG chains/nm$^2$', '0.81 PEG chains/nm$^2$', '1.00 PEG chains/nm$^2$']
        plt.plot(df['Wavelength'], df[cols[i]], label = labels[i])
        plt.xlabel('Wavelength (nm)')
        plt.ylabel('Absorbance')
        plt.xlim([400,800])
        plt.legend()
