"""Helper functions to the GA algorithm."""
...
def conc_to_spectra(conc, stock_spectra):
    """
    Simulate Beer's law and with concentrations of differnt stocks.

    Also requires the basis spectra of these stocks.
...
def fitness(spectra, conc, desired):
    """
    Sort an array by its fitness with the most fit row at the bottom.
...
def select_parents(sorted_array, n_parents):
    ...
    The parents with a higher fitness will have
...
def crossover(parents, n_offspring):
    """
    Randomly create a crossover between parents.

    Perform a crossover between the parents to create offspring that
...
def mutation(array, rate):
    """
    Perform a mutation on some of the values in the offspring array.

    Convert the value to a string and then change one of the
...
def GA_algorithm(x_train_spectra, y_train_conc, x_test, n_parents, n_offspring,
    ...
    """GA algorithm, equivalent of learn function in agent class."""
    np.random.seed(seed)
    ...
    # Performs a mutation on the offspring array
...
def GA_algorithm_unnormalized(x_train_spectra, y_train_conc,
                              ...
                              mutation_rate, mutation_rate_2):
    """The same GA algorithm, without normalization."""
    np.random.seed(seed)
    # Obtains fitness of the input concentrations and their spectra
